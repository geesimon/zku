...
/// @title Ethereum light client deployed on Harmony.
///        This is a SPV-like client that for every Ethereum block, we need to
///        verify validity of the block header and do POW check, then stores 
///        these headers for later usage, i.e., transaction inclusion check.
contract EthereumLightClient is Ethash, Initializable, PausableUpgradeable {
    ...
    /// @dev stores first Ethereum block
    ...
    /// @dev called by relayer to update Ethereum status by giving a block header.
    ///      We need to verify the block header by checking:
    ///         parent hash, block number, timestamp difficulty and POW.
    ///      Then stores block header in `blocks`.
